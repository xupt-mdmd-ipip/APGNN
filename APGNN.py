...
from model.IDE_block import AGNN
...
config_name = 'Indian_pines.py'
# config_name = 'paviaU.py'
# config_name = 'salinas.py'
parser = argparse.ArgumentParser(description="Few Shot Visual Recognition")
parser.add_argument('--config', type=str, default=os.path.join('./config', config_name),
...
def get_train_test_loader(Data_Band_Scaler, GroundTruth, class_num, shot_num_per_class, HalfWidth):
    ...
    test_loader = torch.utils.data.DataLoader(test_dataset, batch_size=4, shuffle=False, num_workers=0)#修改batchsize原本为100，这个100可能与config中n_dim有关
    del test_dataset
    del imdb
...
# seeds = [1230, 1231, 1232, 1233, 1229, 1236, 1335, 1336, 1337, 1338]
# TODO nDataSet
for iDataSet in range(nDataSet):
    ...
    # 一样load target domain data for training and testing 加载目标领域数据集用于训练和测试 这里基本上不需要改动
    ...
    IDE_block_tar = AGNN(config['num_generation'],
                         ...
                         config['distribution_distance_metric'],
                         emb_size).to(GPU)
    print('IDE',IDE_block_tar)
    # IDE_block_src2 = DPGN(config['num_generation'],
    #                      train_opt['dropout'],
    #                      CLASS_NUM * SHOT_NUM_PER_CLASS,
    #                      CLASS_NUM * SHOT_NUM_PER_CLASS + CLASS_NUM * QUERY_NUM_PER_CLASS,
    #                      config['point_distance_metric'],
    #                      config['distribution_distance_metric'],
    #                      emb_size).to(GPU)
    # IDE_block_tar2 = DPGN(config['num_generation'],
    #                       train_opt['dropout'],
    #                       CLASS_NUM * SHOT_NUM_PER_CLASS,
    #                       CLASS_NUM * SHOT_NUM_PER_CLASS + CLASS_NUM * QUERY_NUM_PER_CLASS,
    #                       config['point_distance_metric'],
    ...
    # TODO : CSA_block_att 初始化
    # 多加 CSA_block_att 第一个变量类别数量用了3处，后面三个变量没有用到。 utiis脚本里面的模块函数从这里开始使用 上面是数据处理使用的函数
    ...
    # print('/////////////////////这是query-edge-mask//////////////////////',query_edge_mask.size())


    CSA_block_att = CSA_block.LightweightAttentionalGNN(num_supports, 320, ['self'] *9).to(GPU)
    print('csa:',CSA_block_att)
    # 将以上4个变量进行优化器优化 之后需要修改变量名字
    ...
    if not is_the_first_few_epoch:

        feature_encoder.load_state_dict(torch.load('feature_encoder.pth'))
        IDE_block_src.load_state_dict(torch.load('IDE_block_src.pth'))
        IDE_block_tar.load_state_dict(torch.load('IDE_block_tar.pth'))
        CSA_block_att.load_state_dict(torch.load('CSA_block_att.pth'))
    else:

        # feature_encoder.apply(weights_init)
        IDE_block_src.apply(weights_init)
        IDE_block_tar.apply(weights_init)
        CSA_block_att.apply(weights_init)#修改
    feature_encoder.to(GPU)

    feature_encoder.train()
    IDE_block_src.train()
    IDE_block_tar.train()
    CSA_block_att.train() # 修改
    ...
    for episode in range(EPISODE):
        # get few-shot classification samples ：src and tar 这里基本上不需要改动
        ...
        # DCFSL中只有上面3行，下面三行是本文补加的 ：src and tar 这里基本上不需要改动
        ...
        ''' sample datas扩充成两个域 这里基本上不需要改动 ：src and tar'''
        supports_src, support_labels_src = next(support_dataloader_src.__iter__()) # (5, 100, 9, 9)
        querys_src, query_labels_src = next(query_dataloader_src.__iter__())  # (75,100,9,9)
        ...
        ''' calculate features扩充成两个域 这里基本上不需要改动 ：src and tar'''
        ...
        ''' initialize nodes and edges for dual graph model额外添加 使用到图神经网络的方法 这里基本上不需要改动 ：src and tar'''
        ...
        # TODO : initialize variables DPGN main
        """ initialize variables DPGN main ：src and tar"""
        # initialize variables DPGN main函数中前部分定义的 修改这部分
        tensors_src = utils.allocate_tensors()  # 此函数的功能有待确认 猜测是tensor数据的初始化
        ...
        # 第一行中所有的变量都没用在下文中使用，第二行的变量在后面 run the DPGN model 会使用到
        ...
        ''' calculate prototype network ：src and tar 扩充成两个域 这里基本上不需要改动 '''
        if SHOT_NUM_PER_CLASS > 1:
            ...
            support_proto_src = support_features_src  # few-shot learning for fsl_loss中会使用到
            support_proto_tar = support_features_tar

        ''' the IDE-block ：src and tar 这两部分是新添加的DPGN中main的train函数中有 修改这部分 '''
        # use backbone encode image ：src and tar
        ...
        # TODO : run the DPGN model
        """ run the DPGN model ：src and tar """
        _, _, point_nodes_src, point_edge_src,distribution_nodes_src= IDE_block_src(second_last_layer_data_src,#修改
                                                                         last_layer_data_src,
                                                                         node_feature_gd_src,
                                                                         edge_feature_gd_src,
                                                                         edge_feature_gp_src,
                                                                        query_labels_src)
        _, _, point_nodes_tar, point_edge_tar,distribution_nodes_tar = IDE_block_tar(second_last_layer_data_tar,#修改
                                                                         last_layer_data_tar,
                                                                         node_feature_gd_tar,
                                                                         edge_feature_gd_tar,
                                                                         edge_feature_gp_tar,
        ...
        # distribution_nodes_src = IDE_block_src2(second_last_layer_data_src,
        #                                         last_layer_data_src,
        #                                         node_feature_gd_src,
        #                                         edge_feature_gd_src,
        #                                         edge_feature_gp_src)
        # distribution_nodes_tar = IDE_block_tar2(second_last_layer_data_src,
        #                                         last_layer_data_src,
        #                                         node_feature_gd_src,
        #                                         edge_feature_gd_src,
        #                                         edge_feature_gp_src)
        ...
        # TODO : the CSA-block for cross_att_loss
        """ the CSA-block for cross_att_loss 这两部分是新添加的 """
        last_layer_data_src1 = last_layer_data_src.reshape(320, 64)
        last_layer_data_tar1 = last_layer_data_tar.reshape(320, 64)

        logits_src = euclidean_metric(query_features_src[1], support_proto_src[1])
        logits_src1 = euclidean_metric(last_layer_data_src1, support_proto_src[1])#修改
        ...
        logits_tar1 = euclidean_metric(last_layer_data_tar1, support_proto_src[1])#修改
        f_loss_tar = crossEntropy(logits_tar, query_labels_tar.long().to(GPU))

        # print('logits_src1',logits_src1.size())
        ...
        def dbscan(
                ...
                eps=399,
                *,
                #min_samples=2,#修改：2还是5
                min_samples=400,
                metric="minkowski",
                metric_params=None,
                algorithm="auto",
                leaf_size=30,#修改，原为30
                p=2,
                sample_weight=None,
                n_jobs=None):
            est = DBSCAN(
        ...
        _,labels_1 = dbscan(X=logits_src1.detach().cpu().numpy())
        _, labels_2 = dbscan(X=logits_tar1.detach().cpu().numpy())
        lines_junc_idx_src,lines_junc_idx_tar=[],[]
        b_size = 1
        for bs in range(b_size):
            clusters1 = labels_1
            clusters2 = labels_2
            clusters_src = torch.tensor(clusters1, dtype=torch.long)
            clusters_tar = torch.tensor(clusters2, dtype=torch.long)
            lines_junc_idx_src.append(clusters_src.reshape(-1, 2))
            lines_junc_idx_tar.append(clusters_tar.reshape(-1, 2))
        # print('lines_junc_idx',lines_junc_idx_src)
        # print('lines_junc_idx.size', lines_junc_idx_src.size())
        cross_att_loss = CSA_block_att(point_nodes_src, point_nodes_tar,edge_feature_gp_src,edge_feature_gp_tar, lines_junc_idx_src, lines_junc_idx_tar)#修改

        ''' few-shot learning for fsl_loss ：src and tar'''
        # logits_src = euclidean_metric(query_features_src[1], support_proto_src[1])
        # f_loss_src = crossEntropy(logits_src, query_labels_src.long().to(GPU))
        # logits_tar = euclidean_metric(query_features_tar[1], support_proto_tar[1])
        # f_loss_tar = crossEntropy(logits_tar, query_labels_tar.long().to(GPU))
        ...
        # domain_loss = ot_ep
        ot_ep, wd_ep, gwd_ep = utils.OT(point_nodes_src, point_nodes_tar)
        ot_ed, wd_ed, gwd_ed = utils.OT(distribution_nodes_src, distribution_nodes_tar)#修改

        domain_loss = ot_ep + ot_ed  # 修改

        # TODO loss
        # loss计算和DCFSL不太一样fsl loss domain loss cross_att loss修改这部分
        loss = f_loss + lambda_1 * domain_loss + lambda_2 * cross_att_loss  # 0.01#修改：lambda1和2的值
        # loss = f_loss + domain_loss

        """ Update parameters修改这部分 这里基本上不需要改动"""
        feature_encoder.zero_grad()
        IDE_block_src_optim.zero_grad()
        IDE_block_tar_optim.zero_grad()
        CSA_block_att_optim.zero_grad()#修改
        loss.backward()
        feature_encoder_optim.step()
        IDE_block_src_optim.step()
        IDE_block_tar_optim.step()
        CSA_block_att_optim.step()#修改

        """ 额外扩充成两个域 这一部分分为了源域和目标域 ：src and tar 这里基本上不需要改动"""
        ...
        """ 每进行100轮输出一次结果 这里基本上不需要改动"""
        if (episode + 1) % 100 == 0:
            train_loss.append(loss.item())
            print('episode {:>3d}:  domain loss: {:6.4f}, fsl loss: {:6.4f}, cross att loss: {:6.4f}, acc_src {:6.4f}, '  #修改
                  'acc_tar {:6.4f}, loss: {:6.4f}'.format(
            # print(
            #       'episode {:>3d}:  domain loss: {:6.4f}, fsl loss: {:6.4f}, acc_src {:6.4f}, '
            #       'acc_tar {:6.3f}, loss: {:6.4f}'.format(
                episode + 1,
                domain_loss.item(),
                f_loss.item(),
                cross_att_loss.item(),  # 额外添加#修改
            ...
            print('wd {:6.6f}, gwd: {:6.6f}'.format((wd_ep).item(), (gwd_ep).item()))

        """ 此if判断和DCFSL基本一致 这里基本上不需要改动"""
        if (episode + 1) % 500 == 0 or episode == 0:
            ...
            print("当前运行次数 iDataSet+1:", iDataSet + 1)  # 当前进行的次数
            ...
            if test_accuracy > last_accuracy:
                # TODO save networks .pkl
                torch.save(feature_encoder.state_dict(),
                           str("checkpoints/feature_encoder_" + dataset_name + "_" + str(iDataSet + 1) + "iter_" + str(
                               TEST_LSAMPLE_NUM_PER_CLASS) + "shot.pkl"))  # 不同目标域数据集需要保存名称(从DCFSL中copy)
    ...
    """ 这个if判断是从DCFSL中copy过来的，原文中没有 这里基本上不需要改动 """
...
#每个epoch加载一次模型
torch.save(feature_encoder.state_dict(),'feature_encoder.pth')
torch.save(IDE_block_src.state_dict(),'IDE_block_src.pth')
torch.save(IDE_block_tar.state_dict(), 'IDE_block_tar.pth')
torch.save(CSA_block_att.state_dict(),'CSA_block_att.pth')
""" 下面这一小部分没有做任何改动 这里基本上不需要改动 """
AA = np.mean(A, 1)
...
utils.classification_map(hsi_pic[4:-4, 4:-4, :], best_G[4:-4, 4:-4], 24,
                         "classificationMap" + dataset_name + ".png")
# ----------------------------------------------------------------------------------------------------
