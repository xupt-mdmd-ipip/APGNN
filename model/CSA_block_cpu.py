    ...
    def forward(self, point_nodes_src, point_nodes_tar, distribution_nodes_src, distribution_nodes_tar, point_edge_src,
        ...
        for i in range(len(point_nodes_src)):
            ...
            distribution_nodes_src[i] = torch.tensor(
                preprocessing.scale(distribution_nodes_src[i].squeeze(0).cpu().detach().numpy())).to(
                dev).unsqueeze(0)
            distribution_nodes_tar[i] = torch.tensor(
                preprocessing.scale(distribution_nodes_tar[i].squeeze(0).cpu().detach().numpy())).to(
                dev).unsqueeze(0)
            point_edge_src[i] = torch.tensor(
                preprocessing.scale(point_edge_src[i].squeeze(0).cpu().detach().numpy())).to(
                dev).unsqueeze(0)
            point_edge_tar[i] = torch.tensor(
                preprocessing.scale(point_edge_tar[i].squeeze(0).cpu().detach().numpy())).to(
                dev).unsqueeze(0)
