...
class AGNN(nn.Module):
    def __init__(self, num_generations, dropout, num_support_sample, num_sample, loss_indicator, point_metric, distribution_metric, emb_size):
        """
        AGNN model
        :param num_generations: number of total generations
        :param dropout: dropout rate
    ...
    def forward(self, middle_node, point_node, distribution_node,distribution_edge, point_edge, tr_label):
        ...
        tr_label=tr_label.unsqueeze(0)
        [b, nk]  = tr_label.size()

        tr_label = tr_label.reshape(-1).unsqueeze(1)   #b*1*NK
        # print('这是tr_label.size2：', tr_label.size())
        one_hot = torch.zeros(b*nk,320)#将5全都改成了320
        # print('这是one_hot1',one_hot.size())
        tr_label = tr_label.to(torch.int64)
        one_hot.scatter_(1,tr_label,1)#将1改为了-2
        # print('这是one_hot2', one_hot.size())
        one_hot_fin = one_hot.reshape(b,nk,320)
        # print('one_hot_fin的size', one_hot_fin.size())
        zero_pad = torch.zeros(b, 16,320).fill_(1.0/320) # zero-init or avg-init#修改调参，1/320 还是1/16
        ...
        #Label self-attenion
        lab_new = torch.cat([one_hot_fin,zero_pad], dim=1)#将dim设置成2后 att_l从320,640,640变成了320,320,320
        ...
        new_mask=new_mask.to('cuda:0')
        lab_new=lab_new.to('cuda:0')
        ...
        # print('这是point_node3：', point_node.size())#[1, 320, 384]
        ...
        for l in range(self.generation):
            point_edge, node_similarity_l2 = cp.checkpoint(self._modules['point_sim_generation_{}'.format(l)],point_node, point_edge,
                                                                                                self.point_metric)
            point_node_out = cp.checkpoint(self._modules['distribution2point_generation_{}'.format(l)],point_edge, point_node)
            distribution_node = cp.checkpoint(self._modules['point2distribution_generation_{}'.format(l)],point_edge,
                                                                                            distribution_node)
            distribution_edge = cp.checkpoint(self._modules['distribution_sim_generation_{}'.format(l)],distribution_node,
                                                                                          distribution_edge,
            ...
            distribution_nodes.append(distribution_node)
            point_nodes.append(point_node)
        return point_similarities, node_similarities_l2, point_nodes, point_edge,distribution_nodes
