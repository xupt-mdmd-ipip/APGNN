...
#         print('这是line_enc',line_enc.size())
#         print('这是lines_junc_idx', lines_junc_idx.size())
#         # ldesc is [bs, D, n_junc], line_enc [bs, D, n_lines * 2]    #  1,320,720         1,320,320
...
#         device = ldesc.device###把这四行注释了
#         lines_junc_idx = lines_junc_idx.to(device)
...
#         line_desc = line_desc.to(device)
#         line_enc = line_enc.to(device)
#         message = torch.cat([
#             line_desc,
...
#         message = message.cuda()
#         # b = self.mlp(message)
#         print('这是mlp(message)的大小',self.mlp(message).size())
...
class LineLayer2(nn.Module):
    ...
    def get_endpoint_update(self, ldesc, line_enc, lines_junc_idx):
        ...
        # and lines_junc_idx [bs, n_lines * 2]
        # Create one message per line endpoint
        # print('ldesc',ldesc.size())#1,320,400
        # print('line_enc', line_enc.size())#320,320
        # print('lines_junc_idx1', lines_junc_idx.size())#1,160,2
        lines_junc_idx=lines_junc_idx.reshape(1,320)
        # print('lines_junc_idx2', lines_junc_idx.size())
        # print('lines_junc_idx[:, None]', lines_junc_idx[:, None].size())
        # print('lines_junc_idx[:, None].repeat(1, self.dim, 1)', lines_junc_idx[:, None].reshape(1,320,320).repeat(1, self.dim, 1).size())
        b_size = lines_junc_idx.shape[0]
        lines_junc_idx = lines_junc_idx.type(torch.int64)
        ldesc = ldesc.type(torch.int64)
        print('ldesc', ldesc.size())
        print('lines_junc_idx[:, None].repeat(1, self.dim, 1)', lines_junc_idx[:, None].repeat(1, self.dim, 1).size())
        line_desc = torch.gather(
            ldesc, 2, lines_junc_idx[:, None].repeat(1, self.dim, 1))
        # print('line_desc', line_desc.size())
        # print('line_desc.reshape(1, 320, -1, 2).flip([-1]).flatten(2, 3)', line_desc.reshape(1, 320, -1, 2).flip([-1]).flatten(2, 3).size())
        # print('line_enc', line_enc.size())
        line_enc=line_enc.unsqueeze(0)
        # print('line_desc', line_desc.size())
        # print('line_desc.reshape(1, 320, -1, 2).flip([-1]).flatten(2, 3)', line_desc.reshape(1, 320, -1, 2).flip([-1]).flatten(2, 3).size())
        # print('line_enc', line_enc.size())#都是1,320,320
        message = torch.cat([
            line_desc,
            line_desc.reshape(1, 320, -1, 2).flip([-1]).flatten(2, 3).clone(),#修改：参考484行
            line_enc], dim=1)
        # print('mlp(message)', self.mlp(message).size())
        return self.mlp(message)  # [b_size, D, n_lines * 2]

    def get_endpoint_attention(self, ldesc, line_enc, lines_junc_idx):#这里没用到
        # ldesc is [bs, D, n_junc], line_enc [bs, D, n_lines * 2]
        # and lines_junc_idx [bs, n_lines * 2]
        # print('这是lines_junc_idx:::::1111111111111111111111111111111111111111', lines_junc_idx.size())
        lines_junc_idx = lines_junc_idx.reshape(1, 320)
        lines_junc_idx = lines_junc_idx.type(torch.int64)
        b_size = lines_junc_idx.shape[0]
        expanded_lines_junc_idx = lines_junc_idx[:, None].repeat(1, self.dim, 1)
    ...
    def forward(self, ldesc0, ldesc1, line_enc0, line_enc1, lines_junc_idx0,
        ...
        if self.line_attention:
            ...
            prob0 = self.get_endpoint_attention(ldesc0, line_enc0,
                                                lines_junc_idx0)
            lupdate0 = lupdate0 * prob0[:, None]
            lines_junc_idx0=lines_junc_idx0.type(torch.int64)
            lines_junc_idx1 = lines_junc_idx1.type(torch.int64)
            lines_junc_idx0 = lines_junc_idx0.reshape(1, 320)
            lines_junc_idx1 = lines_junc_idx1.reshape(1, 320)
            print('update0', update0.size())
            print('lines_junc_idx0[:, None].repeat(update0.shape[0], 320, 1)',
                  lines_junc_idx0[:, None].repeat(update0.shape[0], 320, 1).size())

            update0 = update0.scatter_reduce_(
                dim=2, index=lines_junc_idx0[:, None].repeat(update0.shape[0], 320, 1),
                src=lupdate0, reduce='sum', include_self=False)
            prob1 = self.get_endpoint_attention(ldesc1, line_enc1,
                                                lines_junc_idx1)
            lupdate1 = lupdate1 * prob1[:, None]
            update1 = update1.scatter_reduce_(
                dim=2, index=lines_junc_idx1[:, None].repeat(update0.shape[0], 320, 1),
                src=lupdate1, reduce='sum', include_self=False)
        else:
            # Average the updates for each junction (requires torch > 1.12)
            lines_junc_idx0 = lines_junc_idx0.reshape(1,320)
            lines_junc_idx1 = lines_junc_idx1.reshape(1,320)
            # print('lines_junc_idx0[:, None].repeat(1, self.dim, 1)',lines_junc_idx0[:, None].repeat(1, self.dim, 1).size())

            lines_junc_idx0 = lines_junc_idx0.unsqueeze(0).repeat(update0.shape[0], 320, 1)
            lines_junc_idx1 = lines_junc_idx1.unsqueeze(0).repeat(update1.shape[0], 320, 1)
            # print('lines_junc_idx0',lines_junc_idx0.size())
            update0 = update0.scatter_reduce_(dim=2, index=lines_junc_idx0.long(), src=lupdate0, reduce='mean',
                                              include_self=False)
            update1 = update1.scatter_reduce_(dim=2, index=lines_junc_idx1.long(), src=lupdate1, reduce='mean',
                                              include_self=False)

            # print('这是update0::::', update0.size())
            # print('这是update1::::', update0.size())
...
class AttentionalGNN(nn.Module):
    def __init__(self, num_support, feature_dim: int, layer_types: list, checkpointed=False,
        ...
        # print('这是len(enumerate(self.layers))::::::::::::::')  # 10
        # print(len(layers_enum))
        self.line_layers = nn.ModuleList([
            LineLayer2(feature_dim, line_attention)
    ...
    def forward(self, point_nodes_src, point_nodes_tar,edge_feature_gp_src,edge_feature_gp_tar, lines_junc_idx_src, lines_junc_idx_tar):
        global AttentionalGNN_return
        dev = point_nodes_src[0].device
        flag = 0
        for i in range(len(point_nodes_src)):
            # print('len(point_nodes_src):::::', len(point_nodes_src))
            point_nodes_src[i] = torch.tensor(
                preprocessing.scale(point_nodes_src[i].squeeze(0).cpu().detach().numpy())).to(
                dev).unsqueeze(0)
            point_nodes_tar[i] = torch.tensor(
                preprocessing.scale(point_nodes_tar[i].squeeze(0).cpu().detach().numpy())).to(
                dev).unsqueeze(0)
            edge_feature_gp_src[i] = torch.tensor(
                preprocessing.scale(edge_feature_gp_src[i].squeeze(0).cpu().detach().numpy())).to(
                dev).unsqueeze(0)
            edge_feature_gp_tar[i] = torch.tensor(
                preprocessing.scale(edge_feature_gp_tar[i].squeeze(0).cpu().detach().numpy())).to(
                dev).unsqueeze(0)
            # distribution_nodes_src[i] = self.mlp_dis(
            #     torch.tensor(preprocessing.scale(distribution_nodes_src[i].squeeze(0).cpu().detach().numpy())).unsqueeze(
            #         0).transpose(2, 1).to(dev))
            # # print('dis_nodes_src[i]', dis_nodes_src[i].size())
            # distribution_nodes_tar[i] = self.mlp_dis(
            #     torch.tensor(preprocessing.scale(distribution_nodes_tar[i].squeeze(0).cpu().detach().numpy())).unsqueeze(
            #         0).transpose(2, 1).to(dev))
            lines_junc_idx_src[i] = torch.tensor(
                preprocessing.scale(lines_junc_idx_src[i].squeeze(0).cpu().detach().numpy())).to(
                dev).unsqueeze(0)
            lines_junc_idx_tar[i] = torch.tensor(
                preprocessing.scale(lines_junc_idx_tar[i].squeeze(0).cpu().detach().numpy())).to(
                dev).unsqueeze(0)
            # print('point_nodes_src[i]::::::::::::', point_nodes_src[i].size())#1,320,400
            # print('edge_feature_gp_src[i]::::::::::::', edge_feature_gp_src[i].size())  # 320,320
            # print('lines_junc_idx_src[i]::::::::::::', lines_junc_idx_src[i].size())  # 1,160,2
            # print('当前i=', i)
        ...
        for j, layer in enumerate(self.layers):
            ...
            if (layer.type == 'self' and lines_junc_idx_src[flag].shape[1] > 0
                    and lines_junc_idx_tar[flag].shape[1] > 0):
                    ...
                    if self.checkpointed:
                        if 0 <= j < 8:
                            point_nodes_src[flag], point_nodes_tar[flag] = torch.utils.checkpoint.checkpoint(
                                ...
                                point_nodes_src[flag], point_nodes_tar[flag], edge_feature_gp_src[flag],
                                edge_feature_gp_tar[flag], lines_junc_idx_src[flag], lines_junc_idx_tar[flag],
            ...
            # print('这是point_nodes_src[0]::::', point_nodes_src[0].size())
            # print('这是delta0::::', delta0.size())
            # print('这是torch.cat([point_nodes_src[flag], delta0], dim=1)::::', torch.cat([point_nodes_src[flag], delta0], dim=1).size())
            # a=self.mlp(torch.cat([point_nodes_src[flag], delta0], dim=1))
            # b=self.mlp(torch.cat([point_nodes_tar[flag], delta1], dim=1))
            # point_nodes_src[0] = point_nodes_src[0].clone() + a#修改
            # point_nodes_tar[0] = point_nodes_tar[0].clone() + b
            # # print('这是point_nodes_src[0]2::::', point_nodes_src[0].size())
