...
class AGNN(nn.Module):
    def __init__(self, num_generations, dropout, num_support_sample, num_sample, point_metric,emb_size):
        """
        AGNN model
        :param num_generations: number of total generations
        :param dropout: dropout rate
        ...
        for l in range(self.generation):
            D2P = D2PAgg(emb_size * 2, emb_size, dropout=self.dropout if l < self.generation - 1 else 0.0)
            P_Sim = PointSimilarity2(emb_size, emb_size, dropout=self.dropout if l < self.generation - 1 else 0.0)
            self.add_module('distribution2point_generation_{}'.format(l), D2P)
            self.add_module('point_sim_generation_{}'.format(l), P_Sim)
    ...
    def forward(self, middle_node, point_node, lab_vec, point_edge, tr_label):
        ...
        '''修改'''
        att_l = torch.matmul(lab_new, lab_t2)
